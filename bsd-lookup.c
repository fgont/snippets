...
int main(int argc, char *argv[]){
	...
	if(argc < 2){
		puts("usage:  bsd-lookup [-v] IPV6_ADDRESS");
		exit(1);
	}
	else if(argc > 2){
		debug_f= TRUE;
	...
	do{
		...
		if(rtm->rtm_addrs & RTA_GATEWAY){
...
#else
				printf("DEBUG: Family: %d, realsize: %lu\n", sa->sa_family, SA_SIZE(sa));
#endif
			...
			if(sa->sa_family == AF_INET6){
				...
				/* This part of the code tries to guess the struct type based on its size */
				do{
					if(debug_f){
#if defined (__FreeBSD__) || defined(__NetBSD__) || defined (__OpenBSD__) || defined(__APPLE__)
						printf("DEBUG: Family: %d, size %d, realsize: %lu\n", sa->sa_family, sa->sa_len, SA_SIZE(sa));
					...
					SA_NEXT(sa);
				}while(((void *)sa) < end && sa->sa_family != AF_LINK && sa->sa_family != AF_INET6); /* Agregar  && sa->sa_family != AF_LINK && sa->sa_family != AF_INET6 */
